...
class RegimeDetector:
    ...
    def update(self, spread: float):
        ...
        # --- FIXED: Pure exponential discounting instead of baseline reversion ---
        # This removes the floor, allowing beta to shrink when variance drops
        self.kappa = self.lam * self.kappa
        self.alpha = self.lam * self.alpha
        self.beta  = self.lam * self.beta

        # Standard conjugate updates remain the same
